...
namespace Pkcs7lib
{
    ...
    public class Pkcs7SignatureGenerator : IDisposable
    {
        ...
        public Pkcs7SignatureGenerator(string libraryPath, string tokenSerial, string tokenLabel, string pin)
        {
            var ckaLabel = string.Empty;
            byte[] ckaIdValue = null;
            var hashAlgorihtm = HashAlgorithm.SHA256;
            var signatureScheme = SignatureScheme.RSASSA_PKCS1_v1_5;
        ...
        public byte[] GenerateSignature(byte[] data, bool detached, BCX509.X509Certificate signingCertificate, ICollection<BCX509.X509Certificate> certPath)
        {
            ...
            if (detached)
            {
                ...
                return GenerateDetachedSignature(dataHash, hashOid, hashGenerator, signingCertificate, certPath);
            }
            else
                return GenerateAtachedSignature(data, hashOid, hashGenerator, signingCertificate, certPath);
        ...
        /// Generates PKCS#7 signature of specified data
        /// </summary>
        /// <param name="dataHash">Hash Data to be signed</param>
        /// <param name="hashOid">Hash algorithm OID</param>
        /// <param name="hashGenerator">Digest Hash Generator</param>
        /// <param name="signingCertificate">Signing certificate</param>
        /// <param name="certPath">Certification path for signing certificate</param>
        /// <returns>DER encoded PKCS#7 signature of specified data</returns>
        public byte[] GenerateDetachedSignature(byte[] dataHash, string hashOid, IDigest hashGenerator, BCX509.X509Certificate signingCertificate, ICollection<BCX509.X509Certificate> certPath)
        {
            if (this._disposed)
                throw new ObjectDisposedException(this.GetType().FullName);
            ...
            else if (_signatureScheme == SignatureScheme.RSASSA_PSS)
            {
                ...
                using (var session = _slot.OpenSession(SessionType.ReadOnly))
                using (var mechanism = _factories.MechanismFactory.Create(CKM.CKM_RSA_PKCS_PSS, pssMechanismParams))
        ...
        /// <param name="certPath">Certification path for signing certificate</param>
        /// <returns>DER encoded PKCS#7 signature of specified data</returns>
        public byte[] GenerateAtachedSignature(byte[] data, string hashOid, IDigest hashGenerator, BCX509.X509Certificate signingCertificate, ICollection<BCX509.X509Certificate> certPath)
        {
            if (this._disposed)
                throw new ObjectDisposedException(this.GetType().FullName);
            ...
            ContentInfo encapContentInfo = new ContentInfo(
                contentType: new DerObjectIdentifier(OID.PKCS7IdData),
                content: new DerOctetString(data));
        ...
        private IObjectHandle FindPrivateKey(string ckaLabel, byte[] ckaId)
        {
            ...
            {
                var searchTemplate = new List<IObjectAttribute>();
                searchTemplate.Add(_factories.ObjectAttributeFactory.Create(CKA.CKA_CLASS, CKO.CKO_PRIVATE_KEY));
                searchTemplate.Add(_factories.ObjectAttributeFactory.Create(CKA.CKA_KEY_TYPE, CKK.CKK_RSA));
                if (!string.IsNullOrEmpty(ckaLabel))
                    searchTemplate.Add(_factories.ObjectAttributeFactory.Create(CKA.CKA_LABEL, ckaLabel));
                if (ckaId != null)
                    searchTemplate.Add(_factories.ObjectAttributeFactory.Create(CKA.CKA_ID, ckaId));
        ...
        private static ICkRsaPkcsPssParams CreateCkRsaPkcsPssParams(HashAlgorithm hashAlgorithm)
        {
            switch (hashAlgorithm)
            {
                case HashAlgorithm.SHA1:
                    return _factories.MechanismParamsFactory.CreateCkRsaPkcsPssParams(
